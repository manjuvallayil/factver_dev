...
import numpy as np
import faiss
from transformers import RagTokenizer, RagRetriever, RagConfig, RagSequenceForGeneration, AutoTokenizer, AutoModelForSeq2SeqLM
from datasets import load_dataset

class RAGUtils:
    def __init__(self, passages_path, index_path, dataset_vectors_path, llama_model_name='meta-llama/Llama-2-7b-chat-hf', embedding_model_name='sentence-transformers/all-mpnet-base-v2'):
        ...
        # Initialize LLaMA model using AutoModelForSeq2SeqLM instead of AutoModelForCausalLM
        self.llama_tokenizer = AutoTokenizer.from_pretrained(llama_model_name)
        self.llama_model = AutoModelForSeq2SeqLM.from_pretrained(llama_model_name)
    ...
    def create_dataset_and_faiss(self):
        ...
        # Initialize the sentence-transformer model and tokenizer
...
class RAGUtils:
    ...
    def retrieve_evidence(self, claim, aggregated_embedding=None):
        # Tokenize the claim for RAG
        input_ids = self.tokenizer(claim, return_tensors="pt").input_ids
        question_hidden_states = self.rag_model.question_encoder(input_ids)[0]
